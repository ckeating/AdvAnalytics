...
#question - change line type
ggplot(DayHourCounts, aes(x=Hour, y=Freq)) + geom_line(aes(group=Var1),linetype=2)

#question - change line argument to alpha
ggplot(DayHourCounts, aes(x=Hour, y=Freq)) + geom_line(aes(group=Var1),alpha=0.3)
...
#this is to give us the ability to see areas instead of exact locations
...
#question- remove observations with Freq= 0 from LatLonCounts
...
#notes:
# the map on number of murders is simply a population map
# a more meaningful map is the RATE of murders
# first step - create the MurderRate variable
# second step - redo the map
...
#need to plot GunOwnership RATE , not counts
murderMap$GunOwnershipRate = murderMap$GunOwnership / murderMap$Population * 100000

#redo map plot gun ownership RATE
ggplot(murderMap, aes(x = long, y = lat, group = group, fill = murderMap$GunOwnershipRate)) + geom_polygon(color = "black") + scale_fill_gradient(low = "black", high = "red", guide = "legend")

table(murderMap$GunOwnershipRate,murderMap$State)
summary(murderMap)
